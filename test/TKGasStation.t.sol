...
contract TKGasStationTest is Test {
    ...
    function _sign(
        ...
        bytes32 hash = MockDelegate(payable(_publicKey)).hashExecution(_nonce, _to, _value, _args);
        vm.stopPrank();
    ...
    function testETHTransfer() public {
        ...
        vm.prank(paymaster);
        uint256 gasBefore = gasleft();
        bytes memory paramData = abi.encodePacked(signature, bytes16(nonce), "");
    ...
    function testGasComparisonTable() public {
        ...
        bytes memory paramData1 = abi.encodePacked(signature1, bytes16(nonce1), args1);
        ...
        bytes memory paramData2 = abi.encodePacked(signature2, bytes16(nonce2), args2);
        ...
        console.log("| executeNoReturn(param)  | %s | %s |", gasUsed2, gasUsed2 > gasUsed1 ? gasUsed2 - gasUsed1 : gasUsed1 - gasUsed2);
        console.log("+---------------------------------------------------+");
        console.log("");
        
        if (gasUsed1 < gasUsed2) {
            console.log("execute is %s gas more efficient (%s%% savings)", 
                       gasUsed2 - gasUsed1, 
                       ((gasUsed2 - gasUsed1) * 100) / gasUsed2);
        } else {
            console.log("executeNoReturn is %s gas more efficient (%s%% savings)", 
                       gasUsed1 - gasUsed2, 
