...
contract TKGasStationTest is Test {
    ...
    function _sign(
        ...
        (uint8 v, bytes32 r, bytes32 s) =
            vm.sign(_privateKey, TKGasStation(_publicKey).hashExecution(_nonce, _outputContract, _ethAmount, _arguments));
    ...
    function testGassyDelegationInit() public view {
        ...
        assertEq(TKGasStation(user).nonce(), 0);
        assertEq(TKGasStation(user).timeboxedCounter(), 0);
    }
    ...
    function testGassyExecuteSendETH() public {
        ...
        (success, result) = TKGasStation(user).execute(nonce, receiver, 1 ether, "", signature);
...
/*
    ...
    */
    ...
    function testGassyEachUserHasDifferentNonce() public {
        ...
        uint128 nonce = TKGasStation(user).nonce();
        uint128 nonce2 = TKGasStation(user).nonce();
        assertEq(nonce, 0);
        assertEq(nonce2, 0);
        ...
        assertEq(TKGasStation(user).nonce(), nonce + 1);
        assertEq(TKGasStation(user2).nonce(), nonce2);
    }
    ...
    function testGassyExecuteBatch() public {
        ...
        vm.prank(paymaster);
        (success, results) = TKGasStation(user).executeBatch(nonce, executions, signature);
        vm.stopPrank();
    ...
    function testGassyExecuteBatchAttemptToChangeExecution() public {
        ...
        vm.prank(paymaster);
        vm.expectRevert();
        (success, results) = TKGasStation(user).executeBatch(nonce, badExecutions, signature);
        vm.stopPrank();
        ...
        assertEq(TKGasStation(user).nonce(), nonce);
    }
    ...
    function _signBatch(
        ...
        address signer = vm.addr(_privateKey);
        vm.startPrank(signer);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(_privateKey, TKGasStation(_publicKey).hashBatchExecution(_nonce, _executions));
        bytes memory signature = abi.encodePacked(r, s, v);
        vm.stopPrank();
        return signature;
    }
    ...
    function testGassyExecuteBatchSizeLimit() public {
        ...
        vm.prank(paymaster);
        vm.expectRevert();
        TKGasStation(user).executeBatch(nonce, executions, signature);
        vm.stopPrank();
    }
    ...
    function testGassyBurnNonce() public {
        ...
        // Burn the nonce
        vm.prank(paymaster);
        TKGasStation(user).burnNonce(nonce, signature);
        vm.stopPrank();
    ...
    function testGassyBurnNonceRevertsInvalidNonce() public {
        ...
        vm.prank(paymaster);
        vm.expectRevert();
        TKGasStation(user).burnNonce(nonce + 1, signature);
        vm.stopPrank();
    ...
    function testGassyBurnNonceThenExecute() public {
        ...
        vm.prank(paymaster);
        TKGasStation(user).burnNonce(nonce, burnSignature);
        vm.stopPrank();
    ...
    function testGassyDirectBurnNonceVsSignatureBurn() public {
        ...
        uint128 nonceAfterDirect = TKGasStation(user).nonce();
        assertEq(nonceAfterDirect, nonce + 1);

        // Method 2: Signature burn (through TKGasStation)
        uint128 newNonce = TKGasStation(user).nonce();
        bytes memory signature = _signBurnNonce(USER_PRIVATE_KEY, user, newNonce);

        vm.prank(paymaster);
        TKGasStation(user).burnNonce(newNonce, signature);
        vm.stopPrank();

        uint128 nonceAfterSignature = TKGasStation(user).nonce();
        ...
        returns (bytes memory)
    {
        address signer = vm.addr(_privateKey);
        vm.startPrank(signer);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(_privateKey, TKGasStation(_publicKey).hashBurnNonce(_nonce));
        bytes memory signature = abi.encodePacked(r, s, v);
        vm.stopPrank();
        return signature;
    }
    ...
    function _signTimeboxed(
        ...
        vm.startPrank(signer);
        (uint8 v, bytes32 r, bytes32 s) =
            vm.sign(_privateKey, TKGasStation(_publicKey).hashTimeboxedExecution(_counter, _deadline, _sender, _outputContract));
        bytes memory signature = abi.encodePacked(r, s, v);
        vm.stopPrank();
        return signature;
    }
    ...
    function _signTimeboxedArbitrary(
        ...
        vm.startPrank(signer);
        (uint8 v, bytes32 r, bytes32 s) =
            vm.sign(_privateKey, TKGasStation(_publicKey).hashArbitraryTimeboxedExecution(_counter, _deadline, _sender));
        bytes memory signature = abi.encodePacked(r, s, v);
        vm.stopPrank();
        return signature;
    }
    ...
    function _signBurnTimeboxedCounter(
        ...
        vm.startPrank(signer);
        (uint8 v, bytes32 r, bytes32 s) =
            vm.sign(_privateKey, TKGasStation(_publicKey).hashBurnTimeboxedCounter(_counter, _sender));
        bytes memory signature = abi.encodePacked(r, s, v);
        vm.stopPrank();
        return signature;
    }
    ...
    function testExecuteTimeboxed() public {
        uint128 counter = TKGasStation(user).timeboxedCounter();
        ...
        vm.startPrank(paymaster);
        (bool success,) =
            TKGasStation(user).executeTimeboxed(counter, deadline, reciever, ethAmount, executionData, signature);
        vm.stopPrank();

        assertTrue(success);
        assertEq(TKGasStation(user).timeboxedCounter(), 0); // Counter should NOT increment
    ...
    function testExecuteTimeboxedArbitrary() public {
        ...
        vm.startPrank(paymaster);
        (bool success,) =
            TKGasStation(user).executeTimeboxedArbitrary(counter, deadline, reciever, ethAmount, executionData, signature);
        vm.stopPrank();
    ...
    function testExecuteTimeboxedArbitraryRevertsDeadlineExceeded() public {
        ...
        TKGasStation(user).executeTimeboxedArbitrary(
            counter,
    ...
    function testExecuteBatchTimeboxedArbitrary() public {
        ...
        (bool success, bytes[] memory results) =
            TKGasStation(user).executeBatchTimeboxedArbitrary(counter, deadline, executions, signature);
        vm.stopPrank();
    ...
    function testExecuteBatchTimeboxed() public {
        ...
        vm.startPrank(paymaster);
        (bool success, bytes[] memory results) =
            TKGasStation(user).executeBatchTimeboxed(counter, deadline, receiver, executions, signature);
        vm.stopPrank();
    ...
    function testBurnTimeboxedCounter() public {
        ...
        vm.startPrank(paymaster);
        TKGasStation(user).burnTimeboxedCounter(counter, paymaster, signature);
        vm.stopPrank();
