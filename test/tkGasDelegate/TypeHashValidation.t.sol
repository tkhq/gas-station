...
contract TypeHashValidationTest is Test {
    ...
    function testValidateAllTypeHashConstants() public pure {
        ...
        bytes32 expectedExecutionTypehash = keccak256("Execution(uint128 nonce,uint32 deadline,address outputContract,uint256 ethAmount,bytes arguments)");
        bytes32 expectedApproveThenExecuteTypehash = keccak256("ApproveThenExecute(uint128 nonce,uint32 deadline,address erc20Contract,address spender,uint256 approveAmount,address outputContract,uint256 ethAmount,bytes arguments)");
        bytes32 expectedBatchExecutionTypehash = keccak256("BatchExecution(uint128 nonce,uint32 deadline,Call[] calls)Call(address to,uint256 value,bytes data)");
        bytes32 expectedBurnNonceTypehash = keccak256("BurnNonce(uint128 nonce)");
        bytes32 expectedSessionExecutionTypehash = keccak256("SessionExecution(uint128 counter,uint32 deadline,address sender,address outputContract)");
        bytes32 expectedArbitrarySessionExecutionTypehash = keccak256("ArbitrarySessionExecution(uint128 counter,uint32 deadline,address sender)");
        ...
        bytes32 actualArbitrarySessionExecutionTypehash = 0x37c1343675452b4c8f9477fbedff7bcc1e7fa8b3bc97a1e58d4e371c86bd64bb;
        ...
        assertEq(actualApproveThenExecuteTypehash, expectedApproveThenExecuteTypehash, "APPROVE_THEN_EXECUTE_TYPEHASH mismatch");
        ...
        assertEq(actualSessionExecutionTypehash, expectedSessionExecutionTypehash, "SESSION_EXECUTION_TYPEHASH mismatch");
        assertEq(actualArbitrarySessionExecutionTypehash, expectedArbitrarySessionExecutionTypehash, "ARBITRARY_SESSION_EXECUTION_TYPEHASH mismatch");
        assertEq(actualBurnSessionCounterTypehash, expectedBurnSessionCounterTypehash, "BURN_SESSION_COUNTER_TYPEHASH mismatch");
    }
     ...
     */
    function testHashFunctionsUseCorrectTypeHashes() public {
        uint128 nonce = 12345;
     ...
     */
    function testBurnNonceTypeHash() public {
        uint128 nonce = 1;
     ...
     */
    function testSessionExecutionTypeHash() public {
        uint128 counter = 1;
        uint32 deadline = uint32(block.timestamp + 1 days);
        ...
        assertEq(hash, delegate.hashSessionExecution(counter, deadline, sender, outputContract), "SessionExecution hash should be deterministic");
    }
    
    /**
     * @notice Validates ARBITRARY_SESSION_EXECUTION_TYPEHASH
     */
    function testArbitrarySessionExecutionTypeHash() public {
        uint128 counter = 1;
        uint32 deadline = uint32(block.timestamp + 1 days);
        address sender = address(0x1234);
        ...
        assertEq(hash, delegate.hashArbitrarySessionExecution(counter, deadline, sender), "ArbitrarySessionExecution hash should be deterministic");
    }
    
    /**
     * @notice Validates BURN_SESSION_COUNTER_TYPEHASH
     */
    function testBurnSessionCounterTypeHash() public {
        uint128 counter = 1;
        address sender = address(0x1234);
        ...
        assertEq(hash, delegate.hashBurnSessionCounter(counter, sender), "BurnSessionCounter hash should be deterministic");
    }
    
    /**
     * @notice Validates that different inputs produce different hashes (collision resistance check)
     */
    function testHashFunctionsAreCollisionResistant() public {
        uint128 nonce1 = 1;
        uint128 nonce2 = 2;
        uint32 deadline = uint32(block.timestamp + 1 days);
