...
contract TKGasDelegate is IERC1155Receiver, IERC721Receiver {
    ...
    function _execute(address _outContract, bytes calldata _executionData) internal returns (bool, bytes memory) {
    ...
    function _execute(address _outContract, uint256 _ethAmount, bytes calldata _executionData)
        internal
    ...
    function _executeBatch(IBatchExecution.Execution[] calldata _executions) internal returns (bool, bytes[] memory) {
        //if (msg.sender != paymaster) revert NotPaymaster();
