...
contract TKGasDelegate is EIP712, IERC1155Receiver, IERC721Receiver, ITKGasDelegate {
    ...
    fallback(bytes calldata) external returns (bytes memory) {
        ...
        // RETURN PATHS (0xX1) - Checked after no-return paths
            ...
            (, bytes[] memory result) =
                _executeBatchSessionArbitrary(signature, nonceBytes, msg.data[nonceEnd:87], calls);
            return abi.encode(result);
        }
    ...
    function executeSession(address _to, uint256 _ethAmount, bytes calldata _data)
        external
