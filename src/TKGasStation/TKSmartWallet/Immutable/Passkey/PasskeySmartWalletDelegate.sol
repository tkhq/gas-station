...
contract PasskeySmartWalletDelegate is TKGasDelegate {
    ...
    function _validatePasskeySignature(bytes32 _hash, bytes calldata _signature) internal view returns (bool) {
        bytes32 messageHash = sha256(abi.encodePacked(_hash));
        (bytes32 x, bytes32 y) = _getPublicKey();
        bytes memory input = abi.encodePacked(messageHash, _signature[0:32], _signature[32:64], x, y);
